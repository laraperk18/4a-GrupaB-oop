...
##dodatni zadatak
        self.spremi_CSV_gumb = tk.Button(unos_frame, text="Spremi CSV",bg="#67A3D9",fg="white", command=self.spremi_u_csv)
        self.spremi_CSV_gumb.grid(row=4, column=0, padx=5, pady=10, sticky="E")
        self.ucitaj_CSV_gumb = tk.Button(unos_frame, text="Učitaj CSV",bg="#67A3D9",fg="white", command=self.ucitaj_iz_csv)
        self.ucitaj_CSV_gumb.grid(row=4, column=1, padx=5, pady=10, sticky="W")
    ...
    def ucitaj_iz_csv(self):
            with open("ucenici.csv", mode="r", encoding="utf-8") as datoteka:
                ...
                for i in range(0 , len(linije)):
                    red = linije[i].strip().split(",")
                    ime = red[0]
                    prezime = red[1]
                    razred = red[2]
                    u = ucenik(ime, prezime, razred)
