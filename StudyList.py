...
class AktivnostUcenja:
    def __init__(self, predmet, datum, trajanje):
        ...
        try:
            ...
            raise ValueError("Trajanje mora biti pozitivan broj.")
...
class PlanerApp:
    ...
    def spremi_xml(self):
        ...
        for aktivnost in self.aktivnosti:
            ...
            element= ET.SubElement(korijen, tip)
            element.set("predmet", aktivnost.predmet)
            element.set("datum", aktivnost.datum.strftime("%d.%m.%Y"))
            element.set("trajanje", str(aktivnost.trajanje))

            if tip == "TeorijskaSesija":
                element.set("teme", aktivnost.teme)
            else:
                element.set("broj_zadataka", str(aktivnost.brojzad))
                element.set("poteskoca", aktivnost.poteskoca)

        stablo = ET.ElementTree(korijen)
        stablo.write(putanja, encoding="utf-8", xml_declaration=True)
        messagebox.showinfo("Spremanje", "Podaci su spremljeni!")
        self.status_var.set("Datoteka spremljena.")
